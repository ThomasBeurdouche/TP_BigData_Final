...
public class RelationshipRecordReader extends RecordReader<LongWritable, Relationship> {
    ...
    public boolean nextKeyValue() throws IOException, InterruptedException {
        ...
        if (hasNext) {
            ...
            // Lire la valeur (le contenu de la ligne) depuis lineRecordReader
            String line = lineRecordReader.getCurrentValue().toString();

            String[] parts = line.split(","); // A<->B;timestamp
            if (parts.length >= 2) {
                String[] ids = parts[0].split("<->"); //A<->B
                if (ids.length == 2) {
                    currentValue.setId1(ids[0].trim());
                    currentValue.setId2(ids[1].trim());
                }
            }
